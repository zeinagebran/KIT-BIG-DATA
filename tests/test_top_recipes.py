...
def test_format_to_datetime(analysis_instance):
    df = analysis_instance._format_to_datetime(
        analysis_instance.recipes_df, 'submitted')
    assert pd.api.types.is_datetime64_any_dtype(
        df['submitted']), "La colonne 'submitted' devrait être de type datetime"


def test_rename_column(analysis_instance):
    df = analysis_instance._rename_column(
        analysis_instance.recipes_df, 'id', 'recipe_id')
...
def test_merge_with(analysis_instance):
    ...
    merged_df = analysis_instance._merge_with(
        recipes_df, interactions_df, 'recipe_id')
...
def test_format_to_numeric(analysis_instance):
    df = analysis_instance._format_to_numeric(
        analysis_instance.recipes_df, 'minutes')
    assert pd.api.types.is_numeric_dtype(
        df['minutes']), "La colonne 'minutes' devrait être de type numérique"


def test_filter_positive_ratings(analysis_instance):
    filtered_df = analysis_instance._filter_positive_ratings(
        analysis_instance.interactions_df, 'rating', threshold=4)
    assert all(filtered_df['rating'] >=
               4), "Toutes les notations dans le DataFrame filtré devraient être supérieures ou égales à 4"
...
def test_get_top_n_recipes_by_ratings(analysis_instance):
    ...
    assert len(top_recipes['recipe_id'].unique(
    )) <= 2, "Le nombre de recettes obtenues doit être inférieur ou égal à 'n' spécifié"
